 ...
 */ 
    ...
    {
//        if(pFFF_CoreFilterEngine->m_TxID != 0)
        if(pFFF_CoreFilterEngine->InFilter())
        {
...
/* MCHN END */
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CFFFAddress(addr).ToString());
...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be AccepFFF_CoreTransaction
...
int VerifyNewTxForStreamFilters(const CTransaction& tx,std::string &strResult,mc_FFF_CoreFilter **lppFilter,int *applied)            
{
    ...
    if(pFFF_CoreFilterEngine->NoStreamFilters())
    {
        return MC_ERR_NOERROR;
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) 
    {
        ...
        if(result.type() == obj_type)
        {
            ...
            int err=pFFF_CoreFilterEngine->RunStreamFilters(tx,i,(unsigned char*)&hash+MC_AST_SHORT_TXID_OFFSET,-1,0, 
...
//        throw runtime_error("Help message not found\n");
    ...
    if(pFFF_CoreFilterEngine->m_Vout < 0)
    {
    ...
    if(pFFF_CoreFilterEngine->m_Vout >= (int)pFFF_CoreFilterEngine->m_Tx.vout.size())
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "vout out of range");                                            
    }
    Value result=DataItemEntry(pFFF_CoreFilterEngine->m_Tx,pFFF_CoreFilterEngine->m_Vout,streams_already_seen, 0x0E00);
...
//    result.get_obj().push_back(Pair("txid", pFFF_CoreFilterEngine->m_Tx.GetHash().GetHex()));
    result.get_obj().push_back(Pair("vout", pFFF_CoreFilterEngine->m_Vout));
...
Value getfilterstream(const Array& params, bool fHelp)
{
    ...
    Value result=StreamEntry((unsigned char*)&(pFFF_CoreFilterEngine->m_EntityTxID),0x0002);
...
*/    
    Object result;
    TxToJSON(pFFF_CoreFilterEngine->m_Tx, 0, result);
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH(Value& input, inputs) {
            CFFFAddress address(input.get_str());
            if (!address.IsValid())
...
/* MCHN END */       
        ...
        CTxDestination address;
        if (ExtractDestination(txout.scriptPubKey, address)) {
            entry.push_back(Pair("address", CFFFAddress(address).ToString()));
...
/* MCHN START */                
                                                                                // FFF issue #6056 with booust 1.58 fixed in https://github.com/FFF/FFF/commit/8b08d9530b93c7a98e7387167ecd2cd5b0563bfb
...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    {
        if(ParseFFF_CoreTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        if(ParseFFF_CoreTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=FFF_Core");
    }    
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CFFFAddress(CScriptID(script)).ToString()));
    return r;
}
