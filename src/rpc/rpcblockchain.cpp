...
/* MCHN START */
bool ParseFFF_CoreTxOutToBuffer(uint256 hash,const CTxOut& txout,mc_Buffer *amounts,mc_Script *lpScript,int *allowed,int *required,string& strFailReason);
...
/* MCHN START */    
    ...
    if(mc_gState->m_NetworkParams->IsProtocolFFF_Core())
    {
        ...
        {
            miner=CFFFAddress(keyID).ToString();
        }
    }
...
//        throw JSONRPCError(RPC_INVALID_PARAMS, "Wrong number of parameters");                    
    ...
    if( (vin < 0) || (vin >= (unsigned int)pFFF_CoreFilterEngine->m_Tx.vin.size()) )
    {
    ...
    Array getxoutparams;
    getxoutparams.push_back(pFFF_CoreFilterEngine->m_Tx.vin[vin].prevout.hash.ToString());
    getxoutparams.push_back((int64_t)pFFF_CoreFilterEngine->m_Tx.vin[vin].prevout.n);
...
Value gettxout(const Array& params, bool fHelp)
{
    ...
    {
        if (!((CCoinsViewCache*)(pFFF_CoreFilterEngine->m_CoinsCache))->GetCoins(hash, coins))
            return Value::null;        
    }
    else
    {
        if (fMempool) {
            ...
            if (!view.GetCoins(hash, coins))
                return Value::null;
            if(pFFF_CoreFilterEngine->InFilter() == 0)                                // In filter we already checked this input exists, but mempool is dirty
            {
