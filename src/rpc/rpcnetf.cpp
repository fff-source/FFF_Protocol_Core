...
using namespace std;
void PushFFF_CoreRelay(CNode* pto, uint32_t msg_type,vector<CAddress>& path,vector<CAddress>& path_to_follow,vector<unsigned char>& payload);
...
/* MCHN START */        
        if(mc_gState->m_NetworkParams->IsProtocolFFF_Core())
        {
            ...
            {
                obj.push_back(Pair("handshakelocal", CFFFAddress(stats.kAddrLocal).ToString()));                
                obj.push_back(Pair("handshake", CFFFAddress(stats.kAddrRemote).ToString()));                
            }
        }
/* MCHN END */        
...
//                    PushFFF_CoreRelay(pnode, MC_RMT_GLOBAL_PING,path,path_to_follow,payload);
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolFFF_Core())
    {
        obj.push_back(Pair("subversion",
            FormatSubVersion("FFF_Core", mc_gState->GetProtocolVersion(), std::vector<string>())));
    }
    else
    {
        obj.push_back(Pair("subversion",
