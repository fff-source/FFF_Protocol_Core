...
//    obj.push_back(Pair("protocolversion", PROTOCOL_VERSION));
    ...
    obj.push_back(Pair("nodeaddress", FFF_CoreServerAddress(true) + strprintf(":%d",GetListenPort())));       
 ...
 */ 
    ...
    keyID=pkey.GetID();    
    obj.push_back(Pair("handshakelocal",GetArg("-handshakelocal",CFFFAddress(keyID).ToString())));   
...
Value setruntimeparam(const json_spirit::Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "This API is supported only if protocol=FFF_Core");                
    }
...
/* MCHN START */        
    if(!isValid)
    {
        ...
        if(isValid)
        {
            address=CFFFAddress(pubkey.GetID());
        }        
    }
...
Value createkeypairs(const Array& params, bool fHelp)
{
    ...
    for(int i=0;i<count;i++)
    {
        ...
        Object entry;
        entry.push_back(Pair("address", CFFFAddress(pubkey.GetID()).ToString()));
        entry.push_back(Pair("pubkey", HexStr(pubkey)));
        entry.push_back(Pair("privkey", CFFFSecret(secret).ToString()));
...
#ifdef ENABLE_WALLET
        // Case 1: FFF address and we have full public key:
        CFFFAddress address(ks);
