            ...
            if(ptr)
            {
                if( (sighash_type == SIGHASH_ALL) || ( (sighash_type == SIGHASH_SINGLE) && (i == stream_output) ) )
                {
                    ...
                    if(publishers_set.count(publisher_hash) == 0)
                    {
                        ...
                        if(is_redeem_script)
                        {
                            publisher_str=CFFFAddress((CScriptID)publisher_hash).ToString();
                        }
                        else
                        {
                            publisher_str=CFFFAddress((CKeyID)publisher_hash).ToString();                    
                        }
                        if(given_publisher)
                        {
...
//            if( AvailableFromStatus(retrieve_status) && ((retrieve_status & MC_OST_STORAGE_MASK) == MC_OST_OFF_CHAIN ))
            if( AvailableFromStatus(retrieve_status) )
            {
                ...
                {        
                    ...
                    if(pFFF_CoreFilterEngine->RunStreamFilters(wtx,stream_output,(unsigned char *)stream_id,filter_block,filter_offset, 
...
//    int iad=-1;
    ...
    if(fIsFound)
    {
        ...
        int required=0;
        if(ParseFFF_CoreTxOutToBuffer(hash,txout,amounts,lpScript,NULL,&required,strFailReason))
        {
                              ...
                              string *strError)
{
    ...
    {
        *strError="Offchain data is not supported with this wallet version. To get this functionality, run \"FFF_Cored -walletdbversion=2 -rescan\"";
