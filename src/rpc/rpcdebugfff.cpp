...
Value mcd_DebugIssueLicenseToken(const Object& params)
{
    ...
    CFFFAddress from_address(mcd_ParamStringValue(params,"from_address",""));
    if (!from_address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    CFFFAddress to_address(mcd_ParamStringValue(params,"to_address",""));
    if (!to_address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    CFFFAddress req_address(mcd_ParamStringValue(params,"req_address",""));
...
//        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid request type");                    
    ...
    else
    {
        switch(request_type)
        {
            case MC_RMT_MC_ADDRESS_QUERY:
                ...
                CFFFAddress address(addr_to_find);
                CKeyID keyID;
    ...
    if(request_type & MC_RMT_SPECIAL_MASK)
    {
        ...
        for(int a=0;a<attempts;a++)
        {
            ...
            remaining=FFF_CoreCollectChunks(&collector);
            if(remaining == 0)
            {
                return 0;
            }
            ...
            if(params.size() > 3)
            {
                ...
                BOOST_FOREACH(CNode* pnode, vNodes) 
                {
                    ...
                    if( (params[3].get_str() == stats.addrName) || 
                        (params[3].get_str() == CFFFAddress(stats.kAddrRemote).ToString()) )
                    {
                        pto=pnode;
                    }
                }
                    ...
                    case MC_RMT_MC_ADDRESS_QUERY:
                        ...
                        for(int r=0;r<(int)request->m_Responses.size();r++)
                        {
                            if(!res_found)
                            {
                                ...
                                {
                                    Array addresses;
                                    res.push_back(Pair("handshake",CFFFAddress(node_addr.m_Address).ToString()));
