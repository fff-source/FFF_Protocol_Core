...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    bool fNewFound=false;
    vector<string> inputStrings=ParseStringList(params[0]);
    vector<CFFFAddress> inputAddresses;
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        ...
        if (address.IsValid()) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid FFF_Core address or script");
        }
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        CFFFAddress address=inputAddresses[is];
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid FFF_Core address");
    CKeyID keyID;
    ...
    return CFFFSecret(vchSecret).ToString();
}
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by FFF_Core %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CFFFAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            if (pwalletMain->mapAddressBook.count(keyid)) {
                file << strprintf("%s %s label=%s # addr=%s\n", CFFFSecret(key).ToString(), strTime, EncodeDumpString(pwalletMain->mapAddressBook[keyid].name), strAddr);
            } else if (setKeyPool.count(keyid)) {
                file << strprintf("%s %s reserve=1 # addr=%s\n", CFFFSecret(key).ToString(), strTime, strAddr);
            } else {
