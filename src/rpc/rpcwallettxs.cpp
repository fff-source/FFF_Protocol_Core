    ...
    BOOST_FOREACH(const CTxIn& txin, wtx.vin)
    {
        ...
        if (prevout_found)
        {
            if(IsTxOutMineAndInList(pwalletMain,prevout,addresses,filter))
            {
                ...
                if(ParseFFF_CoreTxOutToBuffer(txin.prevout.hash,prevout,amounts,lpScript,NULL,NULL,strFailReason))
                {
    ...
    for (int i = 0; i < (int)wtx.vout.size(); ++i)
    {
        ...
        if(!txout.scriptPubKey.IsUnspendable())
        {
            ...
            if(IsTxOutMineAndInList(pwalletMain,txout,addresses,filter))
            {
                fIsMine=true;
                nIsToMeCount++;
                ParseFFF_CoreTxOutToBuffer(wtx.GetHash(),txout,amounts,lpScript,NULL,&required,strFailReason);
...
//            ParseFFF_CoreTxOutToBuffer(wtx.GetHash(),txout,amounts,lpScript,NULL,&required,strFailReason);
    ...
    if(total)
    {
        ...
        {
            aIssueAddresses.push_back(CFFFAddress(addr).ToString());
        }
    ...
    {
        aMyAddresses.push_back(CFFFAddress(addr).ToString());
    }
    
    BOOST_FOREACH(const CTxDestination& addr, from_addresses)
    {
        aFromAddresses.push_back(CFFFAddress(addr).ToString());
    }
    
    BOOST_FOREACH(const CTxDestination& addr, to_addresses)
    {
//        aToAddresses.push_back(CFFFAddress(addr).ToString());
