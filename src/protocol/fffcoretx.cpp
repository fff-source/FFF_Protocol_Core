...
typedef struct CFFF_CoreTxDetails
{    
...
bool CFFF_CoreTxDetails::IsRelevantInput(int vin, int vout)
{
...
bool CFFF_CoreTxDetails::IsRelevantEntity(uint160 hash)
{
...
void CFFF_CoreTxDetails::SetRelevantEntity(void *entity)
{
    uint160 hash=0;
...
void CFFF_CoreTxDetails::SetAffectedLibrary(void *entity)
{
    uint160 hash=0;
    memcpy(&hash,entity,MC_AST_SHORT_TXID_SIZE);
...
bool mc_CompareAssetQuantities(CFFF_CoreTxDetails *details,string& reason)
{
    unsigned char *ptrIn;
...
void FFF_CoreTransaction_SetTmpOutputScript(const CScript& script1)
{
    CScript::const_iterator pc1 = script1.begin();
    mc_gState->m_TmpScript->Clear();
    mc_gState->m_TmpScript->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),MC_SCR_TYPE_SCRIPTPUBKEY);
...
bool FFF_CoreTransaction_CheckCachedScriptFlag(const CTransaction& tx)
{
    bool flag=false;
...
bool FFF_CoreTransaction_CheckCoinbaseInputs(const CTransaction& tx,     
                                               CFFF_CoreTxDetails *details)
{
    if(!tx.IsCoinBase())                                                  
    {   
        return false;
...
bool FFF_CoreTransaction_CheckInputs(const CTransaction& tx,                  // Tx to check
                                       const CCoinsViewCache &inputs,           // Tx inputs from UTXO database
                                       CFFF_CoreTxDetails *details,           // Tx details object
...
bool FFF_CoreTransaction_CheckNewEntity(int vout,
                                          bool& fScriptParsed,
                                          CFFF_CoreTxDetails *details,   
                                          string& reason)      
{
...
bool FFF_CoreTransaction_CheckCachedScript(const CTransaction& tx, 
...
void FFF_CoreTransaction_FillAdminPermissionsBeforeTx(const CTransaction& tx,
                                                        CFFF_CoreTxDetails *details)      
{
...
bool FFF_CoreTransaction_VerifyAndDeleteDataFormatElements(string& reason,int64_t *total_size,uint32_t *salt_size)
{    
...
bool FFF_CoreTransaction_CheckOpReturnScript(const CTransaction& tx, 
                                               const CCoinsViewCache &inputs,   
                                               int vout,
                                               CFFF_CoreTxDetails *details,   
                                               string& reason)      
{
    ...
    total_offchain_size=0;
    if(!FFF_CoreTransaction_VerifyAndDeleteDataFormatElements(reason,&total_offchain_size,NULL))
    {
        return false;
    }
    ...
    if( mc_gState->m_TmpScript->GetNumElements() == 2 )                         // Cached input script or new entity
    {
        ...
        {
                                                                                // Cached scripts
            if(!FFF_CoreTransaction_CheckCachedScript(tx, inputs, fScriptParsed, details, reason))
            {
                return false;
            }
        }
        ...
        {
                                                                                // New entities
            if(!FFF_CoreTransaction_CheckNewEntity(vout, fScriptParsed, details, reason))
            {
                return false;
            }
        }
                                                                                ...
                                                                                // Performed only if it is indeed needed
    {
        ...
        {
            FFF_CoreTransaction_FillAdminPermissionsBeforeTx(tx,details);
        }                        
    }
...
bool FFF_CoreTransaction_CheckAssetUpdateDetails(mc_EntityDetails *entity,
                                                   int vout,
                                                   CFFF_CoreTxDetails *details,   
                                                   string& reason)      
{
...
bool FFF_CoreTransaction_CheckVariableUpdateDetails(const CTransaction& tx,
                                                   mc_EntityDetails *entity,
                                                   int offset, 
                                                   int vout,
                                                   CFFF_CoreTxDetails *details,   
...
bool FFF_CoreTransaction_CheckUpgradeApproval(const CTransaction& tx,
                                                mc_EntityDetails *entity,
                                                int offset,  
                                                int vout,
                                                CFFF_CoreTxDetails *details,   
...
bool FFF_CoreTransaction_CheckStreamItem(mc_EntityDetails *entity,
                                           int vout,
                                           CFFF_CoreTxDetails *details,   
                                           string& reason)      
{
...
bool FFF_CoreTransaction_CheckEntityItem(const CTransaction& tx,
                                           int offset,  
                                           int vout,
                                           CFFF_CoreTxDetails *details,   
                                           string& reason)      
{
    ...
    if(!FFF_CoreTransaction_VerifyAndDeleteDataFormatElements(reason,NULL,&salt_size))
    {
        return false;
    }
       ...
       (entity.GetEntityType() == MC_ENT_TYPE_LIBRARY))                         // Asset, variable or library update
    {
        if(entity.GetEntityType() == MC_ENT_TYPE_ASSET)
        {
            if(!FFF_CoreTransaction_CheckAssetUpdateDetails(&entity,vout,details,reason))
            {
                return false;            
            }
        }
        ...
        {
            if(!FFF_CoreTransaction_CheckVariableUpdateDetails(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
    ...
    else
    {
        ...
        {
            if(!FFF_CoreTransaction_CheckUpgradeApproval(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
        else                                                                    // (Pseudo)stream item
        {
            if(entity.GetEntityType() <= MC_ENT_TYPE_STREAM_MAX)
            {
                ...
                if(!FFF_CoreTransaction_CheckStreamItem(&entity,vout,details,reason))
                {
                    return false;            
                }            
            }
...
bool FFF_CoreTransaction_CheckDestinations(const CScript& script1,
                                             int vout,
                                             CFFF_CoreTxDetails *details,   
                                             string& reason)      
{
...
bool FFF_CoreTransaction_CheckIfLibraryApproval(unsigned char* ptr,
                                                  CFFF_CoreTxDetails *details)
{
    mc_EntityDetails entity;
...
bool FFF_CoreTransaction_ProcessPermissions(const CTransaction& tx,
                                              int offset,  
                                              int vout,
        ...
        {   
            if(mc_gState->m_TmpScript->GetPermission(&type,&from,&to,&timestamp) == 0) // Grant script
            {
                ...
                if(type)
                {
                    ...
                    {
                        flags=MC_PFL_IS_SCRIPTHASH;
                        address=CFFFAddress(*lpScriptID);
                            ...
                            if(details->vInputDestinations[i] != 0)
                            {
                                if( ( (type & (MC_PTP_ADMIN | MC_PTP_MINE)) == 0) || details->vInputCanGrantAdminMine[i] || (entity.GetEntityType() > 0) )
                                {
                                            ...
                                            from,to,timestamp,flags,1,offset) == 0)
                                    {
                                        if(type & MC_PTP_FILTER)
                                        {
                                            FFF_CoreTransaction_CheckIfLibraryApproval(ptr,details);
                                        }
                                        fAdminFound=true;
                                    }
                                }
...
bool Is_FFF_CoreLicenseTokenTransfer(const CTransaction& tx)
{
    mc_EntityDetails entity;
    if(tx.IsCoinBase())
    {
    ...
    FFF_CoreTransaction_SetTmpOutputScript(tx.vout[0].scriptPubKey);
...
bool FFF_CoreTransaction_CheckLicenseTokenTransfer(const CTransaction& tx,
                                               int unchecked_row, 
                                               CFFF_CoreTxDetails *details,   
                                               string& reason)      
{
...
bool FFF_CoreTransaction_CheckAssetTransfers(const CTransaction& tx,
                                               int offset,  
                                               int vout,
                                               CFFF_CoreTxDetails *details,   
                                               string& reason)      
{
    ...
    if(!FFF_CoreTransaction_CheckLicenseTokenTransfer(tx,unchecked_row,details,reason))
    {
        return false;
    }
...
bool FFF_CoreTransaction_CheckOutputs(const CTransaction& tx,                 // Tx to check
                                        const CCoinsViewCache &inputs,          // Tx inputs from UTXO database
                                        ...
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // Basic checks, destinations and simple grants
    {
        ...
        FFF_CoreTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);

        if(mc_gState->m_TmpScript->IsOpReturnScript())                    
        {
            if(!FFF_CoreTransaction_CheckOpReturnScript(tx,inputs,vout,details,reason))
            {
                return false;
            }
        }
        ...
        {
            if(!FFF_CoreTransaction_CheckDestinations(tx.vout[vout].scriptPubKey,vout,details,reason))
            {
                return false;
            }            
            ...
            if(!FFF_CoreTransaction_ProcessPermissions(tx,offset,vout,permission_type,true,details,reason))
            {
                return false;
            }            
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // create, issue, activate grants (requiring pre-tx permissions)
    {
        if(details->vOutputScriptFlags[vout] & (MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_CREATE | MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_FILTER) )
        {
            ...
            if(!FFF_CoreTransaction_ProcessPermissions(tx,offset,vout,permission_type,false,details,reason))
            {
                return false;
            }                        
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_OP_RETURN_ENTITY_ITEM)
        {
            ...
            if(!FFF_CoreTransaction_CheckEntityItem(tx,offset,vout,details,reason))
            {
                return false;                
            }
...
bool FFF_CoreTransaction_CheckTransfers(const CTransaction& tx,               // Tx to check
                                        int offset,                             // Tx offset in block, -1 if in memppol
                                        CFFF_CoreTxDetails *details,          // Tx details object
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_NOT_OP_RETURN)
        {
            ...
            if(!FFF_CoreTransaction_CheckAssetTransfers(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }        
...
int64_t FFF_CoreTransaction_OffchainFee(int64_t total_offchain_size)            // Total size of offchain items
{
    return (MIN_OFFCHAIN_FEE*total_offchain_size + 999)/ 1000;
}

bool FFF_CoreTransaction_CheckMandatoryFee(CFFF_CoreTxDetails *details,     // Tx details object
                                             int64_t *mandatory_fee,            // Mandatory Fee    
                                             string& reason)                    // Error message
{
    *mandatory_fee = FFF_CoreTransaction_OffchainFee(details->total_offchain_size);
...
bool FFF_CoreTransaction_CheckLicenseTokenDetails(CFFF_CoreTxDetails *details,     // Tx details object
...
bool FFF_CoreTransaction_ProcessAssetIssuance(const CTransaction& tx,         // Tx to check
                                                int offset,                     // Tx offset in block, -1 if in memppol
            ...
            (details->new_entity_type != MC_ENT_TYPE_LIBRARY) )
        {
            ...
            {                    
                if(mc_gState->m_Features->LicenseTokens())
                {
                    ...
                    FFF_CoreTransaction_SetTmpOutputScript(tx.vout[issue_vout].scriptPubKey);
                    if(mc_gState->m_Features->License20010())
                    {
                        if(mc_gState->m_TmpScript->GetNumElements() < 2)
                        {
                    ...
                    if(!FFF_CoreTransaction_CheckLicenseTokenDetails(details,token_address,reason))
                    {
                        return false;                                                                                                                                                        
                    }
...
bool FFF_CoreTransaction_ProcessEntityCreation(const CTransaction& tx,        // Tx to check
                                                 int offset,                    // Tx offset in block, -1 if in memppol
                                                 bool accept,                   // Accept to mempools if successful
                                                 CFFF_CoreTxDetails *details, // Tx details object
                                                 string& reason)                // Error message
...
//                if( (details->new_entity_type <= MC_ENT_TYPE_STREAM_MAX) ||     // Admin persmission is required for upgrades and filters
    ...
    if(update_mempool)
    {
        if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&txid))
        {
            ...
            if(details->new_entity_type == MC_ENT_TYPE_FILTER)
            {
                pFFF_CoreFilterEngine->AddFilter((unsigned char*)&txid+MC_AST_SHORT_TXID_OFFSET,(offset < 0) ? 0 : 1);
            }
        }
        else
        {
...
bool FFF_CoreTransaction_VerifyNotFilteredRestrictions(const CTransaction& tx,        // Tx to check
                                                         int offset,                    // Tx offset in block, -1 if in memppol
                                                         bool accept,                   // Accept to mempools if successful
                                                         CFFF_CoreTxDetails *details, // Tx details object
                                                         string& reason)                // Error message
...
bool FFF_CoreTransaction_VerifyStandardCoinbase(const CTransaction& tx,        // Tx to check
                                                        CFFF_CoreTxDetails *details, // Tx details object
                                                        string& reason)                // Error message
{
    ...
    if(signature_output == (mc_gState->m_NetworkParams->IsProtocolFFF_Core() == 0))
    {
        LogPrintf("Non-standard coinbase: Sigature output doesn't match protocol\n");
        return true;                                                            // Signature output should appear only if protocol=FFF_Core
    }
...
bool FFF_CoreTransaction_ProcessLibraryUpdates(int offset,
                                                 CFFF_CoreTxDetails *details, // Tx details object                                                 
                                                 string& reason)                // Error message
{
    ...
    if(pFFF_CoreFilterEngine)
    {    
        pFFF_CoreFilterEngine->CheckLibraries(&(details->vAffectedLibraries),(offset >= 0));
    }
    return true;
}


bool AcceptFFF_CoreTransaction   (const CTransaction& tx,                     // Tx to check
                                    const CCoinsViewCache &inputs,              // Tx inputs from UTXO database
                                    int offset,                                 // Tx offset in block, -1 if in memppol
                                    ...
                                    uint32_t *replay)                           // Replay flag - if tx should be rechecked or only permissions
{
    ...
    if(mc_gState->m_NetworkParams->IsProtocolFFF_Core() == 0)                 
    {
        return true;
    }    
    
    details.fCheckCachedScript=FFF_CoreTransaction_CheckCachedScriptFlag(tx);
    
    mc_gState->m_TmpAssetsIn->Clear();
    
    if(!FFF_CoreTransaction_CheckCoinbaseInputs(tx,&details))                 // Inputs
    {
        if(!FFF_CoreTransaction_CheckInputs(tx,inputs,&details,reason))
        {
            return false;
        }
    }
    ...
    if(!FFF_CoreTransaction_CheckOutputs(tx,inputs,offset,&details,reason))   // Outputs        
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
    
    if(!FFF_CoreTransaction_ProcessAssetIssuance(tx,offset,accept,&details,reason))                // Asset genesis/followon
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!FFF_CoreTransaction_CheckMandatoryFee(&details,&mandatory_fee,reason))
    {
        fReject=true;
        goto exitlbl;                                                                            
    }                                                                           

    if(!FFF_CoreTransaction_CheckTransfers(tx,offset,&details,reason))        // Transfers and receive permissions
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
                                                                                // Creating of (pseudo)streams/upgrades
    if(!FFF_CoreTransaction_ProcessEntityCreation(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!FFF_CoreTransaction_VerifyNotFilteredRestrictions(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!FFF_CoreTransaction_VerifyStandardCoinbase(tx,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!FFF_CoreTransaction_ProcessLibraryUpdates(offset,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
        ...
        ((flags & MC_AMT_NO_FILTERS) == 0) )
    {
        if(mc_gState->m_Features->Filters())
        {
            if(pFFF_CoreFilterEngine)
            {
                mc_FFF_CoreFilter* lpFilter;
                int applied=0;

                if(pFFF_CoreFilterEngine->RunTxFilters(tx,details.vRelevantEntities,reason,&lpFilter,&applied,(offset >= 0)) != MC_ERR_NOERROR)
                {
...
exitlbl:
    ...
    if(!accept || fReject)                                                      // Rolling back permission database if we were just checking or error occurred    
    {
        ...
        FFF_CoreTransaction_ProcessLibraryUpdates(offset,&details,reason);
    }
