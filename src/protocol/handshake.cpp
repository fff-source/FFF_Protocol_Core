...
bool FFF_CoreNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !FFF_CoreNode_CanConnect(pnode);
    }
...
bool FFF_CoreNode_AcceptData(CNode *pnode)
{
    ...
    return !FFF_CoreNode_DisconnectRemote(pnode);
}
...
bool VerifyFFF_CoreVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifyFFF_CoreVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            LogPrintf("mchn: Connection from %s received on peer=%d in verackack (%s)\n",CFFFAddress(pubKeyHash).ToString().c_str(), pfrom->id,pfrom->addr.ToString());
            if(!FFF_CoreNode_CanConnect(pfrom))
            {
                LogPrintf("mchn: Permission denied for address %s received from peer=%d\n",CFFFAddress(pubKeyHash).ToString().c_str(), pfrom->id);
...
//            if(pfrom->fDefaultMessageStart)
    ...
    {
        CFFFAddress address(mapArgs["-handshakelocal"]);
    ...
    
    {
        ...
        if(pNodeStatus)
        {
            ...
            {
                pNodeStatus->sAddress=CFFFAddress(pkey.GetID()).ToString();
            }
        }
    }
