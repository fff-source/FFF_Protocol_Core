...
uint64_t nLocalFFF_CoreServices = 0;
...
/* MCHN START */
    ...
    nLocalFFF_CoreServices |= pEF->NET_Services();
    if(mc_gState->m_NetworkParams->IsProtocolFFF_Core())
    {
        subver=FormatSubVersion("FFF_Core", mc_gState->GetProtocolVersion(), std::vector<string>());
    }
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
                nLocalHostNonce, subver, nBestHeight, true, nLocalFFF_CoreServices);
/* MCHN END */
}
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (FFF users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "FFF_Core " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. FFF_Core Core is probably already running."), addrBind.ToString());
        else
