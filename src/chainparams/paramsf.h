...
#ifndef FFF_CorePARAMS_H
#define	FFF_CorePARAMS_H
...
typedef struct mc_OneFFF_CoreParam
{    
...
typedef struct mc_FFF_CoreParams
{    
    char *m_lpData;
    mc_MapStringIndex *m_lpIndex;
    mc_OneFFF_CoreParam *m_lpParams;
    int *m_lpCoord;
    int m_Status;
    int m_Size;
    int m_Count;
    int m_IsProtocolFFF_Core;
    int m_ProtocolVersion;
    ...
    int Clone(const char *name,mc_FFF_CoreParams *source);
    ...
    const mc_OneFFF_CoreParam *FindParam(const char *param);
    ...
    int IsParamUpgradeValueInRange(const mc_OneFFF_CoreParam *param,int version,int64_t value);
