...
bool ParseFFF_CoreTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
...
bool ParseFFF_CoreTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
                                  ...
                                  string& strFailReason)                        // OUT error
{
    return ParseFFF_CoreTxOutToBuffer(hash,txout,amounts,lpScript,allowed,required,NULL,strFailReason);
}
...
bool AddressCanReceive(CTxDestination address)
{
    ...
    CFFFAddress addressPrint;
    if(lpKeyID != NULL)
    {
        addressPrint=CFFFAddress(*lpKeyID);
        ptr=(unsigned char*)(lpKeyID);
    }
    else
    {
        addressPrint=CFFFAddress(*lpScriptID);
