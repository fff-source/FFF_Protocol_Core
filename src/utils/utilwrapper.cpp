...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FFF_Core
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FFF_Core
    // Mac and Unix: ~/.FFF_Core
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "FFF_CoreCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FFF_Core";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".FFF_Core-cold";        
    }
    return pathRet / ".FFF_Core";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"FFF_Core",".conf") == 0)
    {
                    ...
                    const char *prefix)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override FFF.conf
                    ...
                    const char *prefix)
{
    try
    {
        ...
        if (!streamConfig.good())
            return MC_ERR_NOERROR; // No FFF.conf file is OK
...
int mc_Params::ReadConfig(const char *network_name)
{
    ...
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"FFF_Core",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc_FFF_CoreParams::SetProtocolGlobals()
{
...
int mc_FFF_CoreParams::SetGlobals()
{
...
int mc_FFF_CoreParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string FFF_CoreServerAddress(bool check_external_ip)
{
