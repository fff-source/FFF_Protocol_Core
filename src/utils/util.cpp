...
#include "FFF_Core/FFF_Core.h"                                              // MCHN
...
/* MCHN START */                            
// Ignoring arguments not starting with "-". original FFF code discard everything after that                
...
/* MCHN START */
static boost::filesystem::path pathCachedFFF_Core;
static boost::filesystem::path pathCachedFFF_CoreLog;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FFF
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FFF
    // Mac: ~/Library/Application Support/FFF
    // Unix: ~/.FFF
/* MCHN START */
    LOCK(csPathCached);
    
    pathCachedFFF_Core=fs::path(string(mc_gState->m_Params->DataDir()));
    fs::path &path =pathCachedFFF_Core;
    return path;
...
/* MCHN START */
    fs::path &path =pathCachedFFF_CoreLog;
    if (!path.empty())
        return path;
    path=fs::path(string(mc_gState->m_Params->DataDir(2,1)));
    return pathCachedFFF_CoreLog;
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "FFF_Core.pid"));
