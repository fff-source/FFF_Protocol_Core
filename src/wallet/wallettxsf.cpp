        ...
        if(lpent->m_EntityType & MC_TET_CHAINPOS)
        {
            switch(lpent->m_EntityType & MC_TET_TYPE_MASK)
            {
                ...
                case MC_TET_SCRIPT_ADDRESS:
                    if(CFFFAddressFromTxEntity(address,lpent))
                    {
...
/*                        
                        else
                        {
                            ...
                            if(pFFF_CoreFilterEngine->RunStreamFilters(tx,i,entity.m_EntityID,filter_block, filter_offset, 
    ...
    else
    {
        if(StoredTxDef.m_Flags & MC_SFL_NODATA)
        {
            if(StoredTxDef.m_Flags & MC_SFL_IS_ADDRESS)
            {
                ...
                {
                    ret=CFFFAddress((CScriptID)subkey_hash).ToString();
                }
                else
                {
                    ret=CFFFAddress((CKeyID)subkey_hash).ToString();                    
                }
            }
        }
        else
