...
int mc_FFF_CoreFilterParams::Init()
{
    Zero();
...
int mc_FFF_CoreFilterParams::Close()
{
    mc_gState->m_Compatibility=m_Compatibility;    
    return MC_ERR_NOERROR;
}

int mc_FFF_CoreFilterParams::Destroy()
{
    return Zero();
}
...
int mc_FFF_CoreLibrary::Destroy()
{
    Zero();
...
int mc_FFF_CoreLibrary::Initialize(const unsigned char* short_txid,uint32_t index)
{
    return MC_ERR_NOERROR;    
}

int mc_FFF_CoreFilter::Zero()
{
...
int mc_FFF_CoreFilter::Initialize(const unsigned char* short_txid)
{
    size_t value_size;
    unsigned char *ptr;
    ...
    if(ptr)
    {
        m_FilterCodeRow=pFFF_CoreFilterEngine->m_CodeLibrary->GetNumElements();
        pFFF_CoreFilterEngine->m_CodeLibrary->AddElement();
        pFFF_CoreFilterEngine->m_CodeLibrary->SetData(ptr,value_size);
        pFFF_CoreFilterEngine->m_CodeLibrary->SetData(&ntc,1);        
    }                                    
    else
    {
        m_FilterCodeRow=pFFF_CoreFilterEngine->m_CodeLibrary->GetNumElements();
...
bool mc_FFF_CoreFilter::HasRelevantEntity(set <uint160>& sRelevantEntities)
{
...
int mc_FFF_CoreFilterEngine::Zero()
{
    m_Filters.clear();
    m_TxID=0;
    m_EntityTxID=0;
...
int mc_FFF_CoreFilterEngine::Destroy()
{
    if(m_Semaphore)
    {
...
void mc_FFF_CoreFilterEngine::Lock(int write_mode)
{        
...
void mc_FFF_CoreFilterEngine::UnLock()
{    
    m_LockedBy=0;
    __US_SemPost(m_Semaphore);
}

int mc_FFF_CoreFilterEngine::InFilter()
{    
...
int mc_FFF_CoreFilterEngine::GetAcceptTimeout()
{    
    return GetArg("-acceptfiltertimeout",DEFAULT_ACCEPT_FILTER_TIMEOUT);
}

int mc_FFF_CoreFilterEngine::GetSendTimeout()
{    
...
int mc_FFF_CoreFilterEngine::SetTimeout(int timeout)
{
    Lock(1);
...
int mc_FFF_CoreFilterEngine::SetTimeoutInternal(int timeout)
{
    for(int i=0;i<(int)m_Filters.size();i++)
    {
        mc_Filter *worker=*(mc_Filter **)m_Workers->GetRow(i);
...
uint160 mc_FFF_CoreFilterEngine::ActiveUpdateID(uint160 hash)
{
    map<uint160,mc_FFF_CoreLibrary>::iterator it=m_Libraries.find(hash);
...
int mc_FFF_CoreFilterEngine::LoadLibrary(uint160 hash,bool *modified)
{
    int count;
    size_t value_size;
    unsigned char *ptr;
    ...
    for(int i=0;i<count;i++)
    {
        ...
        map<uint160,mc_FFF_CoreLibrary>::iterator it=m_Libraries.find(library[i].m_Hash);
        if (it == m_Libraries.end())
        {
            if(i == 0)
            {
...
int mc_FFF_CoreFilterEngine::CheckLibraries(set<uint160>* lpAffectedLibraries, int for_block)
{
    Lock(1);
...
int mc_FFF_CoreFilterEngine::CheckLibrariesInternal(set<uint160>* lpAffectedLibraries, int for_block)
{
    int err;
    bool modified;
    map<uint160,mc_FFF_CoreLibrary>::iterator mit;
    map<uint160,mc_FFF_CoreLibrary>::iterator lit;    
...
mc_Filter *mc_FFF_CoreFilterEngine::StreamFilterWorker(int row,bool *modified)
{
    int err; 
    char *code;
    size_t code_size;
    ...
    *modified=false;
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        ...
        map<uint160,mc_FFF_CoreLibrary>::iterator it=m_Libraries.find(mc_LibraryIDForUpdate(m_Filters[row].m_Libraries[i]));
        if(it == m_Libraries.end())
        {
...
int mc_FFF_CoreFilterEngine::RebuildFilter(int row,int for_block)
{
    int err; 
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        map<uint160,mc_FFF_CoreLibrary>::iterator it=m_Libraries.find(m_Filters[row].m_Libraries[i]);
        if (it != m_Libraries.end())
        {
...
int mc_FFF_CoreFilterEngine::AddFilter(const unsigned char* short_txid,int for_block)
{
    Lock(1);
...
int mc_FFF_CoreFilterEngine::AddFilterInternal(const unsigned char* short_txid,int for_block)
{    
    int err;
    mc_FFF_CoreFilter filter;
...
int mc_FFF_CoreFilterEngine::Reset(int block,int for_block)
{
    Lock(1);
...
int mc_FFF_CoreFilterEngine::ResetInternal(int block,int for_block)
{
    int filter_block;
    int err;
...
int mc_FFF_CoreFilterEngine::NoStreamFilters()
{
    int ret=1;
...
int mc_FFF_CoreFilterEngine::RunStreamFilters(const CTransaction& tx,int vout, unsigned char *stream_short_txid,int block,int offset,std::string &strResult,mc_FFF_CoreFilter **lppFilter,int *applied)            
{
    if(mc_gState->m_Features->StreamFilters() == 0)
    {
        return MC_ERR_NOERROR;
...
int mc_FFF_CoreFilterEngine::RunTxFilters(const CTransaction& tx,std::set <uint160>& sRelevantEntities,std::string &strResult,mc_FFF_CoreFilter **lppFilter,int *applied,bool only_once)
{    
    Lock(0);
...
int mc_FFF_CoreFilterEngine::RunFilter(const CTransaction& tx,mc_Filter *filter,std::string &strResult)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
int mc_FFF_CoreFilterEngine::RunFilterWithCallbackLog(const CTransaction& tx,int vout,uint256 stream_txid,mc_Filter *filter,std::string &strResult, json_spirit::Array& callbacks)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
void mc_FFF_CoreFilterEngine::SetCallbackNames()
{
    Lock(1);
...
void mc_FFF_CoreFilterEngine::SetCallbackNamesInternal()
{
    m_CallbackNames.clear();
...
int mc_FFF_CoreFilterEngine::Initialize()
{
    mc_Buffer *filters;
    unsigned char *txid;
    int err=MC_ERR_NOERROR;
