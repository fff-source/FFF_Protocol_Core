...
namespace
{
class CFFFAddressVisitor : public boost::static_visitor<bool>
{
private:
    CFFFAddress* addr;

public:
    CFFFAddressVisitor(CFFFAddress* addrIn) : addr(addrIn) {}
...
/* MCHN START */
bool CFFFAddress::Set(const CKeyID &id,const std::vector<unsigned char>& vchV)
{
    SetData(vchV, &id, 20);
    return true;    
}
...
bool CFFFAddress::Set(const CKeyID& id)
{
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CFFFAddress::Set(const CScriptID& id)
{
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CFFFAddress::Set(const CTxDestination& dest)
{
    return boost::apply_visitor(CFFFAddressVisitor(this), dest);
}

bool CFFFAddress::IsValid() const
{
    return IsValid(Params());
}

bool CFFFAddress::IsValid(const CChainParams& params) const
{
...
CTxDestination CFFFAddress::Get() const
{
    if (!IsValid())
...
bool CFFFAddress::GetKeyID(CKeyID& keyID) const
{
...
bool CFFFAddress::GetScriptID(CScriptID& scriptID) const
{
...
bool CFFFAddress::IsScript() const
{
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CFFFSecret::SetKey(const CKey& vchSecret)
{
...
bool CFFFSecret::SetString(const char* pszSecret)
{
...
bool CFFFSecret::SetString(const std::string& strSecret)
{
