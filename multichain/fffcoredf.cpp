...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/FFF.conf are parsed in qt/FFF.cpp's main()
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling FFF_Cored. Exiting...\n\n",mapArgs["-datadir"].c_str());
        delete mc_gState;
        return false;        
    }
    ...
    {
        fprintf(stderr,"\nError: Log directory %s needs to exist before calling FFF_Cored. Exiting...\n\n",mapArgs["-logdir"].c_str());
        return false;        
    }
...
*/
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  FFF_Cored <blockchain-name> [options]                     " + _("Start FFF_Core Core Daemon") + "\n";

            strUsage += "\n" + HelpMessage(HMM_FFFD);                       // MCHN-TODO edit help message
        }
...
//                delete mc_gState;
    ...
    if(err)
    {
        fprintf(stderr,"ERROR: Couldn't read configuration file for blockchain %s. Please try upgrading FFF_Core. Exiting...\n",mc_gState->m_Params->NetworkName());
        delete pEF;
        delete mc_gState;                
        return false;
    }
    ...
    if(err)
    {
        ...
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Please restart FFF_Cored with reindex=1.\n",mc_gState->m_Params->NetworkName());                        
        }
        else
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Probably FFF_Cored for this blockchain is already running. Exiting...\n",mc_gState->m_Params->NetworkName());
        }
        delete pEF;
        delete mc_gState;                
        return false;
    ...
    if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
    {
        ...
        if(mc_gState->IsSupported(protocol_version) == 0) 
        {
            ...
            {
                fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in FFF_Core %s\n\n",
            ...
            {
                fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by FFF_Core running newer protocol version (%d)\n\n",
                        mc_gState->m_Params->NetworkName(),protocol_version);                        
                fprintf(stderr,"Please upgrade FFF_Core\n\n");
                delete pEF;
                delete mc_gState;                
                return false;
            }
    ...
    switch(mc_gState->m_NetworkParams->m_Status)
    {
        ...
        case MC_PRM_STATUS_MINIMAL:
            if(mc_gState->GetSeedNode() == NULL)
            {
                ...
                fprintf(stderr,"  FFF_Core-util create %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  FFF_Core-util clone <old-blockchain-name> %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"\nAnd rerun FFF_Cored %s\n\n\n",mc_gState->m_Params->NetworkName());                        
                fprintf(stderr,"If you want to connect to existing blockchain please specify seed node:\n\n");
                fprintf(stderr,"  FFF_Cored %s@<seed-node-ip>\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  FFF_Cored %s@<seed-node-ip>:<seed-node-port>\n\n\n",mc_gState->m_Params->NetworkName());
                delete pEF;
                delete mc_gState;                
                return false;
            }
        ...
        case MC_PRM_STATUS_INVALID:
            ...
            fprintf(stderr,"  FFF_Core-util clone %s <new-blockchain-name>\n",mc_gState->m_Params->NetworkName());
            delete pEF;
            delete mc_gState;                
            return false;
...
/*        
        ...
        if (fDaemon)
        {
            fprintf(stdout, "FFF_Core server starting\n");
