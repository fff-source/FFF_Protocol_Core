...
def get_options():
    mc_path_default = str(Path.home() / "FFF_Core")
    ...
    parser.add_argument("-m", "--FFF_Core", metavar="DIR", default=mc_path_default,
                        help="FFF_Core path prefix (default: %(default)s)")
    ...
    if not Path(options.FFF_Core).exists():
        parser.error("{!r}: FFF_Core path does not exist".format(options.FFF_Core))

    logger.info("{} - {}".format(logger.name, parser.description))
    logger.info("  Path:     {}".format(options.FFF_Core))
...
def main():
    ...
    options = get_options()
    os.chdir(str(Path(options.FFF_Core) / "v8build" / "v8" / "out.gn" / "x64.release"))
