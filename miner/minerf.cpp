...
//                *canMine=mc_gState->m_Permissions->CanMine(NULL,pubkey_hash);
                ...
                *canMine=mc_gState->m_Permissions->CanMine(NULL,&pubkey_hash);
                if((*canMine & MC_PTP_MINE) == 0)
                {
                    ...
                    {
                        LogPrintf("mchn: FFF_CoreMiner: cannot mine now, waiting...\n");
                    }
                    testValidity=false;
                }
                else
                {
                    ...
                    {
                        LogPrintf("CreateNewBlock(): total size %u\n", nBlockSize);
                        LogPrintf("mchn: FFF_CoreMiner: Starting mining...\n");
                    }                    
                }
            }            
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("FFF_CoreMiner : mining is paused, generated block is dropped");            
        }
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            return error("FFF_CoreMiner : generated block is stale");
        }
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FFF_CoreMiner : ProcessNewBlock, block not accepted");
...
double GetMinerAndExpectedMiningStartTime(CWallet *pwallet,CPubKey *lpkMiner,set <CTxDestination> *lpsMinerPool,double *lpdMiningStartTime,double *lpdActiveMiners,uint256 *lphLastBlockHash,int *lpnMemPoolSize,double wAvBlockTime)
{
    ...
    *lpdMiningStartTime=mc_TimeNowAsDouble() + 0.5 * dTargetSpacing;        
    ...
    if(!fWaitingForMiner)
    {
        ...
        {
            CFFFAddress addr=CFFFAddress(kThisMiner.GetID());
...
void static FFFMiner(CWallet *pwallet)
{
    LogPrintf("FFF_CoreMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("FFF-miner");
...
/* MCHN START */            
            ...
            if (Params().MiningRequiresPeers() 
                    ...
                    || (mc_gState->m_NetworkParams->IsProtocolFFF_Core() == 0)
                    )
                    ) {
...
/* MCHN END */    
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in FFF_CoreMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce,pwallet);

            LogPrint("mcminer","mchn-miner: Running FFF_CoreMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        if(mc_gState->m_ProtocolVersionToUpgrade > mc_gState->m_NetworkParams->ProtocolVersion())
                        if( (mc_gState->m_ProtocolVersionToUpgrade > 0) && (mc_gState->IsSupported(mc_gState->m_ProtocolVersionToUpgrade) == 0) )
                        {
                            LogPrintf("FFF_CoreMiner: Waiting for upgrade, block is dropped\n");
                        }
                        else
                        {
...
/* MCHN END */    
    ...
    {
        LogPrintf("FFF_CoreMiner terminated\n");
        throw;
    }
}

void GenerateFFFs(bool fGenerate, CWallet* pwallet, int nThreads)
{
...
/* MCHN END */        
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FFFMiner, pwallet));
}
