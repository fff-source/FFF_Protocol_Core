...
bool ProcessFFF_CoreRelay(CNode* pfrom, CDataStream& vRecv, CValidationState &state);
bool ProcessFFF_CoreVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushFFF_CoreVerack(CNode* pfrom, bool fIsVerackack);
bool FFF_CoreNode_CanConnect(CNode *pnode);
bool FFF_CoreNode_DisconnectRemote(CNode *pnode);
bool FFF_CoreNode_DisconnectLocal(CNode *pnode);
bool FFF_CoreNode_RespondToGetData(CNode *pnode);
bool FFF_CoreNode_SendInv(CNode *pnode);
bool FFF_CoreNode_AcceptData(CNode *pnode);
bool FFF_CoreNode_IgnoreIncoming(CNode *pnode);
bool FFF_CoreNode_IsLocal(CNode *pnode);
bool FFF_CoreNode_CollectChunks();
...
#if defined(NDEBUG)
# error "FFF cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "FFF_Core Signed Message:\n";
...
int SetUpgradedParamValue(const mc_OneFFF_CoreParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        SetFFF_CoreParam("targetblocktime",value);
    }   
...
int FFF_CoreNode_ApplyUpgrades(int current_height)
{
    ...
    SetFFF_CoreParams();            
    if(pFFF_CoreFilterEngine)                                                 // Added from version 20012. 
    {
        pFFF_CoreFilterEngine->SetCallbackNames();
    }
...
void FFF_CoreNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool FFF_CoreNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(!AcceptFFF_CoreTransaction(tx,view,-1,MC_AMT_DEFAULT,reason, &mandatory_fee, &replay))
        {
            return state.DoS(0,
                             error("AcceptToMemoryPool: : AcceptFFF_CoreTransaction failed %s : %s", hash.ToString(),reason),
                             REJECT_NONSTANDARD, reason);
        }
...
*/
        ...
        CAmount nFees = 0;
        int async_count=0;
        bool fIsLicenseTokenTransfer=Is_FFF_CoreLicenseTokenTransfer(tx);
...
/* MCHN START */        
    if(!fJustCheck)
    {
        ...
        {
            pFFF_CoreFilterEngine->Reset(pindex->nHeight-1,1);
        }
    }
/* MCHN END */        
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            const CTransaction &tx = block.vtx[i];
            string reason;
            if(!AcceptFFF_CoreTransaction(tx,view,offset,MC_AMT_DEFAULT,reason,NULL,NULL))
            {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                ...
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptFFF_CoreTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
                }
            }
/* MCHN END */                    
...
/* MCHN START */        
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(!AcceptFFF_CoreTransaction(tx,view,coinbase_offset,MC_AMT_DEFAULT,reason,NULL,NULL))
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptFFF_CoreTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    {
        CFFFAddress addr=CFFFAddress(chainActive.Tip()->kMiner.GetID());
...
/* MCHN START */    
    ...
    if(pFFF_CoreFilterEngine)
    {
        pFFF_CoreFilterEngine->Reset(old_height-1,0);
    }
    
    FFF_CoreNode_ApplyUpgrades(old_height-1);        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
/*    
    if(pFFF_CoreFilterEngine)
    {
        pFFF_CoreFilterEngine->Reset(pindexNew->nHeight-1,1);
    }
 */ 
    ...
    }
    {
        ...
        if(pFFF_CoreFilterEngine)
        {
            pFFF_CoreFilterEngine->m_CoinsCache=&view;
        }
        bool rv = ConnectBlock(*pblock, state, pindexNew, view);        
        if(pFFF_CoreFilterEngine)
        {
            pFFF_CoreFilterEngine->m_CoinsCache=NULL;
        }
        if(rv)
        {
...
/* MCHN START */        
    ...
    FFF_CoreNode_ApplyUpgrades(chainActive.Height());    
/* MCHN END */    
...
//    if (!fInvalidFound)
    {
        ...
        if(pFFF_CoreFilterEngine)
        {
            pFFF_CoreFilterEngine->Reset(chainActive.Height(),0);
        }
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    FFF_CoreNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id, CBlockIndex *pindexChecked)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            FFF_CoreNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */
    ...
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
        ...
        if(pwalletTxsMain->RollBack(NULL,chainActive.Height()) != MC_ERR_NOERROR)
        {
            ...
            {
                strError="Error: The wallet database is inconsistent. Restart FFF_Core with -rescan to rebuild the wallet database from the blockchain (can take hours), or with -skipwalletchecks to continue operating anyway";
                return false;
            }
        }
    }
...
/* MCHN END */        
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            if(pFFF_CoreFilterEngine)
            {
                pFFF_CoreFilterEngine->m_CoinsCache=&coins;
            }            
...
/* MCHN START */    
    if(!FFF_CoreNode_RespondToGetData(pfrom)) 
    {
...
//    RandAddSeedPerfmon();
    ...
    if (strCommand == "version")
    {
        ...
        pfrom->nFFF_CoreServices=0;
        if (!vRecv.empty())
        {
            vRecv >> pfrom->nFFF_CoreServices;
        }
...
/* MCHN START */
        ...
        if(mc_gState->m_NetworkParams->IsProtocolFFF_Core())
        {            
            if(GetBoolArg("-FFFstylehandshake", false))
            {
                LogPrintf("mchn: FFF-style-handshake, sending empty verack to peer %d... \n", pfrom->id);
                pfrom->fParameterSetVerified=true;
                pfrom->PushMessage("verack");                
            }
            else
            {
                PushFFF_CoreVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolFFF_Core())
        {
            if(vRecv.size() == 0)
            {
                ...
                else
                {
                    ...
                    {
                        LogPrintf("mchn: FFF-style verack received from peer %d, parameter set NOT VERIFIED, connecting... \n", pfrom->id);
                        pfrom->fParameterSetVerified=true;                    
                        CompleteProcessVersion(pfrom);                
                    }
                }
            ...
            else
            {
                if(GetBoolArg("-FFFstylehandshake", false))
                {
                    LogPrintf("mchn: FFF-style-handshake, ignoring verack from peer %d \n", pfrom->id);                    
                }
                else
                {
                    bool disconnect_flag=false;
                    if(!ProcessFFF_CoreVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushFFF_CoreVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolFFF_Core())
        {
            ...
            if(!ProcessFFF_CoreVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushFFF_CoreVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!FFF_CoreNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !FFF_CoreNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && FFF_CoreNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!FFF_CoreNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && FFF_CoreNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && FFF_CoreNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(FFF_CoreNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
                    LogPrintf("mchn: Address %s lost connect permission on peer=%d, diconnecting...\n",CFFFAddress(pfrom->kAddrRemote).ToString().c_str(), pfrom->id);            

            }
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && FFF_CoreNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
                        LogPrintf("mchn: Local address %s lost connect permission. disconnecting peer %d...\n",CFFFAddress(pfrom->kAddrLocal).ToString().c_str(), pfrom->id);            
                }
            }
        }
    }
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !FFF_CoreNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(FFF_CoreNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=FFF_CoreNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && FFF_CoreNode_IsBlockChainSynced(pto))
            {
...
/* MCHN START */        
        ...
        if(pwalletTxsMain->m_ChunkCollector)
        {
            ...
            {
                if(FFF_CoreNode_CollectChunks())
                {
                    FFF_CoreCollectChunks(pwalletTxsMain->m_ChunkCollector);
                }                
                pwalletTxsMain->m_ChunkCollector->m_NextTryTimestamp=time_millis_now+FFF_CoreCollectChunksQueueStats(pwalletTxsMain->m_ChunkCollector);
