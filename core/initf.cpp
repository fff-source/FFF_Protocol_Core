...
mc_FFF_CoreFilterEngine* pFFF_CoreFilterEngine = NULL;
...
void Shutdown()
{
    ...
    RenameThread("FFF-shutoff");
    mempool.AddTransactionsUpdated(1);
    StopRPCThreads();
#ifdef ENABLE_WALLET
    if (pwalletMain)
        bitdbwrap.Flush(false);
    GenerateFFFs(false, NULL, 0);
#endif
    StopNode();
...
/* MCHN START */  
    ...
    {
        delete pFFF_CoreFilterEngine;
        pFFF_CoreFilterEngine=NULL;        
    }
...
std::string HelpMessage(HelpMessageMode mode)                                   // MCHN
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "FFF_Core.conf") + "\n";
    if (mode == HMM_FFFD)
    {
#if !defined(WIN32)
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "FFF_Core.pid") + "\n";
#endif
    strUsage += "  -reindex               " + _("Rebuild the blockchain and reindex transactions on startup.") + "\n";
#if !defined(WIN32)
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default FFF_Core address otherwise") + "\n";
#endif
/* MCHN START */    
/* Default was 0 */    
...
//    strUsage += "  -blockprioritysize=<n> " + strprintf(_("Set maximum size of high-priority/low-fee transactions in bytes (default: %d)"), DEFAULT_BLOCK_PRIORITY_SIZE) + "\n";
    ...
    strUsage += "\n" + _("FFF_Core runtime parameters") + "\n";    
    strUsage += "  -offline                                 " + _("Start FFF_Cored in offline mode, no connections to other nodes.") + "\n";
    strUsage += "  -storeruntimeparams                      " + _("Permanently save modifications to runtime parameters made by setruntimeparam APIs.") + "\n";
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running FFF_Cored for the first time.") + "\n";
    strUsage += "  -handshakelocal=<address>                " + _("Manually override the wallet address which is used for handshaking with other peers in a FFF_Core blockchain.") + "\n";
    ...
    strUsage += "  -explorersupport=0|1                     " + _("Provide support for FFF_Core Explorer 2, default 0") + "\n";

    strUsage += "\n" + _("FFF_Core API response parameters") + "\n";        
    strUsage += "  -hideknownopdrops      " + strprintf(_("Remove recognized FFF_Core OP_DROP metadata from the responses to JSON-RPC calls (default: %u)"), 0) + "\n";
    ...
    strUsage += "  -v1apicompatible       " + strprintf(_("JSON-RPC calls responses compatible with FFF_Core 1.0 (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(_("Copyright (c) Coin Sciences Ltd - www.FFF_Core.com")) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Full terms are shown at: http://www.FFF_Core.com/terms-of-service/")) +
           "\n";
    
/*    
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The FFF Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that FFF is running in a usable environment with all
        ...
        if(pwalletMain)
        {
            if(pwalletMain->vchDefaultKey.IsValid())
            {
                LogPrintf("mchn: Minimal blockchain parameter set is created, default address: %s\n",
                        CFFFAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                ...
                {    
                    ...
                    sprintf(bufOutput,"FFF_Core-cli %s grant %s connect\n",mc_gState->m_NetworkParams->Name(),
                         CFFFAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                    sprintf(bufOutput,"FFF_Core-cli %s grant %s connect,send,receive\n\n",mc_gState->m_NetworkParams->Name(),
                         CFFFAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                }
                else
...
/* MCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FFF_Core Core is shutting down."));
...
#endif
    // Make sure only a single FFF process is using the data directory.
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FFF_Core Core is probably already running."), strDataDir));
#endif
/* MCHN END */
...
//#endif
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
//    LogPrintf("FFF version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolFFF_Core())
    {
        LogPrintf("FFF_Core version build %s protocol %s (%s)\n", mc_BuildDescription(mc_gState->GetNumericVersion()), mc_gState->GetProtocolVersion(), CLIENT_DATE);
    }
    ...
    if (!fDisableWallet) {
        ...
        if (filesystem::exists(pathWalletDat))
        {
            ...
            if(currentwalletdatversion != 1)
            {
                ...
                {
                    return InitError(strprintf("Wallet version %d is not supported in this edition of FFF_Core.\n"
                            "To upgrade to version %d, run FFF_Core Offline Daemon: \n"
                            "FFF_Cored-cold %s -datadir=%s -walletdbversion=3\n"
                            "and restart FFF_Cored or use Community Edition.\n",
        ...
        switch(currentwalletdatversion)
        {
            ...
            case 2:
                break;
            case 1:
                return InitError(strprintf("Wallet version 1 is not supported in this version of FFF_Core. "
                        "To upgrade to version 2, run FFF_Core 1.0: \n"
                        "FFF_Cored %s -walletdbversion=2 -rescan\n",mc_gState->m_NetworkParams->Name()));                                        
            default:
...
//            if (!CWalletDB::Recover(bitdbwrap, strWalletFile, true))
            if(!WalletDBRecover(bitdbwrap,strWalletFile,true))
                return false;
            sprintf(bufOutput,"\nTo work properly with salvaged addresses, you have to call importaddress API and restart FFF_Core with -rescan\n\n");
...
//        uiInterface.InitMessage.connect(SetRPCWarmupStatus);
    ...
    uiInterface.InitMessage(_("Initializing FFF_Core..."));
    ...
    {    
        ...
        if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
        {
            ...
            return InitError(_("wallet.dat is partially corrupted. Please try running FFF_Core with -salvagewallet."));                            
        }
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL)
            {
                ...
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        ...
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to FFF_Core 1.0 beta 2 or later"));                                                        
                        }
...
*/
            ...
            if(mc_QuerySeed(seedThreadGroup,seed_resolved.c_str()))
            {
                ...
                {
                    seed_error="Couldn't disconnect from the seed node, please restart FFF_Cored";
...
//                    if(seed_port.size() == 0)
                    ...
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d - please check FFF_Cored is running at that address and that your firewall settings allow incoming connections.",                
                            seed_ip.c_str(),seed_port);
                        pNodeStatus->sLastError="Couldn't connect to the seed node, FFF_Cored is not running or firewall issue";
                    }
                }
            }
            else
            ...
            if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
            {
                ...
                if(mc_gState->IsSupported(protocol_version) == 0) 
                {
                    ...
                    {
                        seed_error=strprintf("The protocol version (%d) for blockchain %s has been deprecated and was last supported in FFF_Core %s\n",                
                    ...
                    else
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d.\n"
                                    "Blockchain was created by FFF_Cored with newer protocol version (%d)\n"                
                                    "Please upgrade to the latest version of FFF_Core or connect only to blockchains using protocol version %d or earlier.\n",                
            ...
            if(mc_gState->m_NetworkState == MC_NTS_SEED_NO_PARAMS)
            {
                ...
                seed_error=strprintf("Couldn't retrieve blockchain parameters from the seed node %s on port %d.\n"
                            "For FFF protocol blockchains, the file %s must be copied manually from an existing node.",                
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
            {
                SelectFFF_CoreParams(mc_gState->m_Params->NetworkName());
        ...
        else
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_GENERATED)
            {
                ...
                mc_gState->m_NetworkParams->SetGlobals();                           // Needed to update IsProtocolFFF_Core flag in case of FFF
    ...
    if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
    {
        ...
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart FFF_Cored with reindex=1.\n",mc_gState->m_Params->NetworkName());
...
*/
                ...
                {
                    return InitError("Wallet tx database corrupted. Please restart FFF_Cored with -rescan\n");                        
                }
                ...
                {                    
                    ...
                    {
                        return InitError("Explorer database was initialized with different setting. To change it, please restart FFF_Cored with -rescan\n");                                                    
                    }
                }
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart FFF_Cored with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        
                ...
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart FFF_Cored with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after FFF_Core parameters were initizalized
        {
            pwalletMain = new CWallet(strWalletFile);
            DBErrors nLoadWalletRetForBuild = pwalletMain->LoadWallet(fFirstRunForBuild);
            ...
            if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
            {
                ...
                return InitError(_("wallet.dat corrupted. Please try running FFF_Core with -salvagewallet."));                            
            }
        ...
        if(pwalletMain)
        {
            ...
            if(new_wallet_txs)
            {
                ...
                BOOST_FOREACH(const PAIRTYPE(CFFFAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
                {
                    const CFFFAddress& address = item.first;
...
/* MCHN START */    
    ...
    pFFF_CoreFilterEngine=new mc_FFF_CoreFilterEngine;
    if(pFFF_CoreFilterEngine->Initialize())
    {
    ...
    if(!GetBoolArg("-shortoutput", false))
    {
        if(fListen && !GetBoolArg("-offline",false))
        {
            ...
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            sprintf(bufOutput,"FFF_Cored %s:%d\n\n",FFF_CoreServerAddress(false).c_str(),GetListenPort());
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            if(found_ips > 1)
            {
        ...
        else
        {
            if(GetBoolArg("-offline",false))
            {                
                sprintf(bufOutput,"FFF_Core started in offline mode, other nodes cannot connect.\n\n");
                bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));            
            }
            else
            {
    ...
    {
        sprintf(bufOutput,"%s:%d\n",FFF_CoreServerAddress(true).c_str(),GetListenPort());                
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
    }
...
/* MCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart FFF_Cored with reindex=1."),
...
//    int version=mc_gState->m_NetworkParams->GetInt64Param("protocolversion");
    ...
    LogPrintf("FFF_Core protocol version: %d\n",version);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of FFF_Core Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // MCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart FFF_Core Core to complete") << "\n";
...
//    RandAddSeedPerfmon();
    ...
    // Generate coins in the background
        if (pwalletMain)
            GenerateFFFs(GetBoolArg("-gen", true), pwalletMain, GetArg("-genproclimit", 1));
#endif
